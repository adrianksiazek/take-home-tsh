...
export const Product = ({ name, promo, image, description, rating, active }: ProductProps) => {
  ...
  return (
    <>
      ...
      <div className="flex max-w-md flex-col justify-between rounded-lg bg-white">
        <div className="relative">
          <img src={image} alt="Product" className="max-h-52 w-full rounded-t-lg" loading="lazy" />
        ...
        <div className="flex h-full flex-col justify-between px-4 pb-6 pt-3">
          ...
          <div>
            ...
            {!active ? (
              <Button className="btn-primary btn-lg mt-4 w-full" disabled>
                Unavailable
              </Button>
            ) : (
              <Button className="btn-primary btn-lg mt-4 w-full" onClick={handleToggleModal}>
                Show details
              </Button>
            )}
          </div>
